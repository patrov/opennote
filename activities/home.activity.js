define(["Kimo/core","ReadList.ContentTypePluginMng"], function(Kimo,ContentTypePluginManager) {
        ...
        Kimo.ActivityManager.createActivity("HomeActivity", {
            ...
            showHomeAction: function(params) {
                ...
                this.templateItem = '<div id="{{_cid}}" data-uid="{{_cid}}" class="item"><h3 class="title" data-field-name=""><i class="fa fa-file-text-o fa-1x"></i> {{title}}</h3><span class="author">{{author}}</span><span>, {{publisher}} <em>{{place}}</em> {{year}}</span></div>';
                this.listView.render(content);
                frag.appendChild($(content).get(0));
                this.view.setContent($(frag));
            },
            ...
            _createDataList: function() {
                ...
                var itemRender = function(entity) {
                    return Mustache.render(self.templateItem, entity);
                }

                return new Kimo.DataView({
                    data: allData,
                    appendToTop: true,
